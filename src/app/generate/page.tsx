...
import { saveRoadmap, SaveRoadmapInput } from '@/ai/flows/save-roadmap';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from '@/components/ui/card';
import { Form, FormControl, FormDescription, FormField, FormItem, FormLabel, FormMessage } from '@/components/ui/form';
import { Input } from '@/components/ui/input';
import { Checkbox } from '@/components/ui/checkbox';
import { Loader2, Wand2, ArrowRight, BrainCircuit, Briefcase, PlusCircle, Sparkles, LucideIcon, ListTodo, BookOpen, Lightbulb, Code, Milestone, Database, Server, XIcon, CheckCircle, Upload, Lock, Save, LogOut } from 'lucide-react';
...
export default function GenerateRoadmapPage() {
  ...
  const handleSaveRoadmap = async () => {
    if (!roadmapData || !userInput || !user) return;
    setSaving(true);
    try {
      // We can't send File objects to the backend flow.
      // So we create a version of roadmapData that is serializable.
      const serializableRoadmapData = {
        ...roadmapData,
        roadmap: roadmapData.roadmap.map(step => {
          const { certificate, ...rest } = step;
          return {
            ...rest,
            // We can store certificate info here if needed, e.g., name or a storage path
            certificateName: certificate?.name || null 
          };
        })
      };
      
      const saveData: SaveRoadmapInput = {
          userId: user.uid,
          userInput,
          roadmapData: serializableRoadmapData,
      }
      
      await saveRoadmap(saveData);
             ...
             <div className="space-y-8 w-full">
               <div className="text-center mb-8">
                ...
                <div className="flex justify-end mb-4">
                    <Button onClick={handleSaveRoadmap} disabled={saving}>
